...
# Admin - clean up topics, recreate topics

def createTopics(conf):

    topics = [config['General']['factTopic'], config['General']['dimTopic']]

    # delete topics
    a = AdminClient(conf['Kafka'])

    a.delete_topics(topics, operation_timeout=10)
